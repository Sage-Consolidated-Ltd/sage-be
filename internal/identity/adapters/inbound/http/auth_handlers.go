...
type AuthHandler struct {
	authServ    inbound.AuthUseCase
	oAuthConfig *config.OAuthConfig
	appConfig   *config.APIConfig
}

func NewAuthHandler(authServ inbound.AuthUseCase, appConfig *config.APIConfig) *AuthHandler {
...
func (a *AuthHandler) CreateUser(c *fiber.Ctx) error {
	...
	if !resp.IsVerified {
		...
		return response.JSON(c, fiber.StatusOK, "User and organization created successfully. Please verify your email.", nil)
	}
...
func (a *AuthHandler) AuthCallback(c *fiber.Ctx) error {
	...
	resp, err := a.authServ.OAuthLogin(ctx, &dto.CreateUserRequest{
	...
	if len(resp.Organization) > 0 {
		...
		if orgID == "" && resp.Organization[0].ID != "" {
			orgID = resp.Organization[0].ID
			if resp.Organization[0].Role != "" {
				userRole = resp.Organization[0].Role
			}
		}
	}
...
func (a *AuthHandler) Enable2FA(c *fiber.Ctx) error {
	...
	var req dto.GoogleAuthenticatorRequest
	if err := c.BodyParser(&req); err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error(), nil)
	}
...
func (a *AuthHandler) Logout(c *fiber.Ctx) error {
	...
	if err == nil && sess != nil {
		if err := sess.Destroy(); err != nil {
			logger.Error("Error destroying session during logout: ", zap.Error(err))
		}
	}

	c.ClearCookie("session_id", "oauth_state")
