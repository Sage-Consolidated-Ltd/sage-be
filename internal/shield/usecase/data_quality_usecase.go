...
type DataQualityService struct {
	scanRepo   outbound.DataQualityRepository
...
func (s *DataQualityService) GetSummary(ctx context.Context, orgID uuid.UUID) (*domain.DataQualityScan, error) {
...
func (s *DataQualityService) GetBreakdown(ctx context.Context, orgID uuid.UUID, scanID *uuid.UUID, page, pageSize int) ([]*domain.DataQualitySourceMetric, error) {
	if scanID == nil {
		// Use latest scan
		...
		if scan == nil || scan.ID == uuid.Nil {
			return []*domain.DataQualitySourceMetric{}, nil
		}
		scanID = &scan.ID
	}
...
func (s *DataQualityService) GetAIAnalysis(ctx context.Context, orgID uuid.UUID, scanID *uuid.UUID) ([]map[string]interface{}, error) {
	...
	suggestions, err := s.scanRepo.GetSuggestions(ctx, orgID, nil, nil, domain.SuggestionStatusPending)
	if err != nil {
		return nil, err
	}
...
func (s *DataQualityService) ApplySuggestedFix(ctx context.Context, orgID uuid.UUID, suggestionID uuid.UUID) error {
	...
	var target *domain.DataQualitySuggestion
	for _, sug := range suggestions {
	...
	return s.scanRepo.UpdateSuggestionStatus(ctx, suggestionID, orgID, domain.SuggestionStatusApplied)
}
...
func (s *DataQualityService) GetSuggestedFixDiff(ctx context.Context, suggestionID, parserID uuid.UUID) (map[string]interface{}, error) {
	// Retrieve suggestion
	suggestions, err := s.scanRepo.GetSuggestions(ctx, uuid.Nil, &parserID, nil, domain.SuggestionStatusPending)
	if err != nil {
		return nil, err
	}
	var target *domain.DataQualitySuggestion
