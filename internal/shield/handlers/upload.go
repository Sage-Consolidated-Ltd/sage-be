...
type UploadHandler struct {
	uploadService services.UploadServiceInt
}

func NewUploadHandler(uploadService services.UploadServiceInt) *UploadHandler {
...
func (u *UploadHandler) UploadLog(c *fiber.Ctx) error {
	...
	var req requests.UploadLogRequest
...
func (u *UploadHandler) UploadComplete(c *fiber.Ctx) error {
	var req requests.UploadCompleteRequest
	...
	_, err := u.uploadService.ValidateUploadComplete(
		c.Context(),
		&rc,
		&req,
	)
	...
	return response.JSON(c, fiber.StatusOK, "UPLOAD_COMPLETE", nil)
}
