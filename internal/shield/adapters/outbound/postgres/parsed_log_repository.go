...
func NewParsedLogRepository(database *db.DB) outbound.ParsedLogRepository {
	return &ParsedLogRepository{
		db: database,
	}
}

func buildBulkInsertParsedLogsQuery(logs []domain.ParsedLog) (string, []any, error) {
	const colsPerRow = 7
...
func (p *ParsedLogRepository) StoreParsedLogs(ctx context.Context, logs []domain.ParsedLog) error {
	if len(logs) == 0 {
		return nil
	}
...
func (p *ParsedLogRepository) ReplaceParsedLogs(ctx context.Context, fileID uuid.UUID, logs []domain.ParsedLog) error {
...
func (r *ParsedLogRepository) Search(ctx context.Context, params domain.SearchParams) (domain.SearchResult, error) {
	...
	return domain.SearchResult{Logs: logs, NextCursor: next}, nil
}

func buildSearchQuery(params domain.SearchParams) (string, []any, error) {
