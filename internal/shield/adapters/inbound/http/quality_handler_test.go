...
func (m *mockDataQualityService) GetSummary(ctx context.Context, orgID uuid.UUID) (*domain.DataQualityScan, error) {
...
func (m *mockDataQualityService) GetBreakdown(ctx context.Context, orgID uuid.UUID, sourceID *uuid.UUID, page, pageSize int) ([]*domain.DataQualitySourceMetric, error) {
	args := m.Called(ctx, orgID, sourceID, page, pageSize)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]*domain.DataQualitySourceMetric), args.Error(1)
}
...
func TestQualityHandler_GetDataQualityBreakdown_Success(t *testing.T) {
	...
	orgID := uuid.New()
	metrics := []*domain.DataQualitySourceMetric{
...
func TestQualityHandler_ApplySuggestedFix_Success(t *testing.T) {
	...
	req := dto.ApplySuggestedFixRequest{
