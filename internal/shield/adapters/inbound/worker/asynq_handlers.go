	...
	g.Go(func() error {
		...
		if h.qualityEngine != nil {
			for _, evt := range securityEvents {
				if qr, err := h.qualityEngine.EvaluateEvent(ctx, evt); err == nil && qr != nil {
					if evt.NormalizedPayload == nil {
						evt.NormalizedPayload = make(map[string]interface{})
					}
					evt.NormalizedPayload["_quality_score"] = qr.Score
					if qr.Status == types.QualityError {
						evt.ParseStatus = types.ParseStatusFailed
					} else if qr.Status == types.QualityPartial && evt.ParseStatus == types.ParseStatusSuccess {
						evt.ParseStatus = types.ParseStatusPartial
					}
				}
			}
		}
		...
		log.Printf("Stored %d security events for file %s", len(securityEvents), filename)
		return nil
	})
...
func (h *TaskHandler) HandleQualityScanJob(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &payload); err != nil {
		return fmt.Errorf("failed to unmarshal quality scan payload: %w", err)
	}
	...
	scan.Status = "running"
	if err := h.qualityRepo.UpdateScan(ctx, scan); err != nil {
		return err
	}

	if h.qualityEngine == nil || h.qualityRepo == nil {
		errMsg := "quality engine or repository not configured"
		_ = h.jobRepo.UpdateJobStatus(ctx, job.ID, job.OrganizationID, domain.JobStatusFailed, 0, 0, &errMsg)
		scan.Status = "failed"
		scan.CompletedAt = &now
		_ = h.qualityRepo.UpdateScan(ctx, scan)
		return fmt.Errorf("%s", errMsg)
	}

	events, _, err := h.eventRepo.SearchEvents(ctx, payload.OrganizationID, nil, 1, 1000)
	if err != nil {
		errStr := err.Error()
		_ = h.jobRepo.UpdateJobStatus(ctx, job.ID, job.OrganizationID, domain.JobStatusFailed, 0, 0, &errStr)
		scan.Status = "failed"
		scan.CompletedAt = &now
		_ = h.qualityRepo.UpdateScan(ctx, scan)
		return fmt.Errorf("failed to search events for quality scan: %w", err)
	}
	...
	if len(events) == 0 {
		...
		scan.QualityScore = &qualityScore
		scan.ParsingErrors = &zero
		scan.MissingFieldsPercentage = &zeroFloat
		scan.DuplicateEventsPercentage = &zeroFloat
		scan.UnmappedLogsCount = &zero
		scan.Status = "completed"
		scan.CompletedAt = &completedAt
		summary := "No events found to evaluate. Quality baseline is 100%."
		...
		job.Status = domain.JobStatusCompleted
		job.CompletedAt = &completedAt
		return h.jobRepo.UpdateJobStatus(ctx, job.ID, job.OrganizationID, job.Status, job.EventsProcessed, job.EventsFailed, job.ErrorMessage)
	}

	results, err := h.qualityEngine.EvaluateBatch(ctx, events)
	if err != nil {
		errStr := err.Error()
		_ = h.jobRepo.UpdateJobStatus(ctx, job.ID, job.OrganizationID, domain.JobStatusFailed, 0, 0, &errStr)
		scan.Status = "failed"
		scan.CompletedAt = &completedAt
		_ = h.qualityRepo.UpdateScan(ctx, scan)
		return fmt.Errorf("failed to evaluate events batch: %w", err)
	}

	var totalScore float64 = 0
	var totalParsingErrors int64 = 0
	var eventsWithMissingFields int = 0
	var eventsWithDuplicates int = 0
	var totalUnmapped int64 = 0
	...
	sourceMap := make(map[uuid.UUID]*sourceStats)
	...
	for i, evt := range events {
		...
		for _, issue := range res.Issues {
			if issue.Category == "missing_field" || issue.Category == "missing_metadata" {
		...
		isParseError := evt.ParseStatus == types.ParseStatusFailed || len(evt.ParseErrors) > 0
		if isParseError {
			totalParsingErrors++
		}
		if hasMissingField {
		...
		isUnmapped := evt.SourceID == uuid.Nil || evt.EventType == "unmapped" || evt.ParseStatus == types.ParseStatusFailed
		if isUnmapped {
			totalUnmapped++
		}

		if evt.SourceID != uuid.Nil {
			sID := evt.SourceID
			stats, exists := sourceMap[sID]
			if !exists {
				stats = &sourceStats{sourceID: sID}
				sourceMap[sID] = stats
			}
			stats.totalEvents++
	...
	avgScore := int(math.Round(totalScore / n))
	missingFieldsPct := math.Round((float64(eventsWithMissingFields)/n)*10000) / 100
	duplicatePct := math.Round((float64(eventsWithDuplicates)/n)*10000) / 100

	for _, stats := range sourceMap {
		sn := float64(stats.totalEvents)
		srcAvg := stats.totalScore / sn
		srcStatus := types.QualityGood
		if srcAvg < 40 {
			srcStatus = types.QualityError
		} else if srcAvg < 60 {
			srcStatus = types.QualityPartial
		} else if srcAvg < 80 {
			srcStatus = types.QualityWarning
		}

		srcMissingPct := math.Round((float64(stats.missingFieldsCount)/sn)*10000) / 100
		srcDupPct := math.Round((float64(stats.duplicateCount)/sn)*10000) / 100

		metric := &domain.DataQualitySourceMetric{
			OrganizationID:          payload.OrganizationID,
			ScanID:                  payload.ScanID,
			SourceID:                stats.sourceID,
			ParsingErrors:           stats.parsingErrors,
			MissingFieldsPercentage: srcMissingPct,
			UnmappedEvents:          stats.unmappedCount,
			DuplicatePercentage:     srcDupPct,
			Status:                  srcStatus,
		}
		if err := h.qualityRepo.CreateSourceMetric(ctx, metric); err != nil {
			logger.Warn("Failed to persist source metric during quality scan", zap.Error(err), zap.String("source_id", stats.sourceID.String()))
		}
	}

	scan.QualityScore = &avgScore
	scan.ParsingErrors = &totalParsingErrors
	scan.MissingFieldsPercentage = &missingFieldsPct
	scan.DuplicateEventsPercentage = &duplicatePct
	scan.UnmappedLogsCount = &totalUnmapped
	scan.Status = "completed"
	scan.CompletedAt = &completedAt
	summary := fmt.Sprintf("Evaluated %d events. Overall quality score: %d/100 across %d sources.", len(events), avgScore, len(sourceMap))
	scan.AISummary = &summary
	if err := h.qualityRepo.UpdateScan(ctx, scan); err != nil {
		return fmt.Errorf("failed to update scan results: %w", err)
	}

	job.EventsProcessed = int64(len(events))
	job.Status = domain.JobStatusCompleted
	job.CompletedAt = &completedAt
	if err := h.jobRepo.UpdateJobStatus(ctx, job.ID, job.OrganizationID, job.Status, job.EventsProcessed, job.EventsFailed, job.ErrorMessage); err != nil {
	...
	log.Printf("Successfully completed quality scan job %s for scan %s (evaluated %d events, score: %d)", job.ID, scan.ID, len(events), avgScore)
	return nil
}
...
func (h *TaskHandler) HandleProviderEventBatch(ctx context.Context, t *asynq.Task) error {
	...
	for _, ev := range payload.Events {
		...
		security_events = append(security_events, &domain.SecurityEvent{
			...
			NormalizedPayload: map[string]interface{}{"user_id": re.UserID, "_ingest_type": "polled", "provider": re.Provider},
	...
	if h.qualityEngine != nil {
		for _, evt := range security_events {
			if qr, err := h.qualityEngine.EvaluateEvent(ctx, evt); err == nil && qr != nil {
				if evt.NormalizedPayload == nil {
					evt.NormalizedPayload = make(map[string]interface{})
				}
				...
				if qr.Status == types.QualityError {
					evt.ParseStatus = types.ParseStatusFailed
				} else if qr.Status == types.QualityPartial && evt.ParseStatus == types.ParseStatusPending {
					evt.ParseStatus = types.ParseStatusPartial
				}
			}
		}
