...
func NewUsersServices(usersRepo outbound.UserRepository, profileRepo outbound.ProfileRepository, redisClient *redis.Client, uploader *s3.Uploader) UserUseCase {
	return &UserServices{
		userRepo:    usersRepo,
...
func (s *UserServices) GetProfile(ctx context.Context, userID string) (*domain.GetUserResponse, error) {
...
func (s *UserServices) GetIdentity(ctx context.Context, userID, orgID string) (*domain.ProfileResponse, error) {
	user, err := s.userRepo.GetUserByID(ctx, userID)
	if err != nil {
		return nil, err
	}
...
func (s *UserServices) GetPreferences(ctx context.Context, userID, orgID string) (*domain.UserPreferencesResponse, error) {
...
func (s *UserServices) GetNotifications(ctx context.Context, userID, orgID string) (*domain.UserNotificationsResponse, error) {
...
func (s *UserServices) GetSessions(ctx context.Context, userID, orgID, currentSessionID string) ([]*domain.UserSessionResponse, error) {
	sessions, err := s.profileRepo.GetUserSessions(ctx, userID, orgID)
	if err != nil {
		return nil, err
	}

	var responses []*domain.UserSessionResponse
...
func (s *UserServices) GetActivity(ctx context.Context, userID, orgID string, page, pageSize int) ([]*domain.UserActivityResponse, int, error) {
	logs, total, err := s.profileRepo.GetUserActivity(ctx, userID, orgID, page, pageSize)
	if err != nil {
		return nil, 0, err
	}

	var responses []*domain.UserActivityResponse
