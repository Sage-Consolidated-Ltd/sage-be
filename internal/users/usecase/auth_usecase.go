...
func (s *AuthService) CreateUserWithOrganization(ctx context.Context, req *dto.OnboardingRequest) (*domain.GetUserResponse, error) {
	// check if email already exists
	user, err := s.userRepo.GetUserByEmail(ctx, req.Email)
	if user != nil {
...
func (s *AuthService) OAuthLogin(ctx context.Context, payload *dto.CreateUserRequest) (*domain.GetUserResponse, error) {
...
func (s *AuthService) Login(ctx context.Context, req *dto.LoginRequest) (*domain.GetUserResponse, error) {
	user, err := s.userRepo.GetUserByEmail(ctx, req.Email)
	if err != nil {
		return nil, err
	}
