...
type CompanyHandler struct {
	companyServ inbound.CompanyUseCase
}

func NewCompanyHandler(companyServ inbound.CompanyUseCase) *CompanyHandler {
...
func (h *CompanyHandler) GetIndustries(c *fiber.Ctx) error {
	industries, err := h.companyServ.GetIndustries(c.Context())
	if err != nil {
...
func (h *CompanyHandler) GetOrganizationRoles(c *fiber.Ctx) error {
	roles, err := h.companyServ.GetOrganizationRoles(c.Context())
	if err != nil {
...
func (h *CompanyHandler) InviteMember(c *fiber.Ctx) error {
	...
	var req dto.BulkInviteMembersRequest
...
func (h *CompanyHandler) InviteMembers(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&req); err != nil || len(req.Invites) == 0 {
		var singleReq dto.MemberInvite
		if errSingle := c.BodyParser(&singleReq); errSingle == nil && singleReq.Email != "" {
			req.Invites = []dto.MemberInvite{singleReq}
		} else if err != nil {
			return response.Error(c, fiber.StatusBadRequest, err.Error(), nil)
		}
	}
...
// ResetMemberMFA resets a member's 2FA authentication
...
func (h *CompanyHandler) ResetMemberMFA(c *fiber.Ctx) error {
	...
	if err := h.companyServ.ResetMemberMFA(c.Context(), memberID, orgID, role); err != nil {
...
// UpdateMemberStatus updates a member's status (active, suspended, pending)
...
func (h *CompanyHandler) UpdateMemberStatus(c *fiber.Ctx) error {
	...
	if err := h.companyServ.UpdateMemberStatus(c.Context(), memberID, orgID, role, req.Status); err != nil {
...
// GetOrganizationProfile returns the organization profile (company details, environment metadata, branding)
func (h *CompanyHandler) GetOrganizationProfile(c *fiber.Ctx) error {
	orgID, _, _, ok := middlewares.GetSessionInfo(c)
	if !ok {
		return response.Error(c, fiber.StatusUnauthorized, "unauthorized", nil)
	}

	profile, err := h.companyServ.GetOrganizationProfile(c.Context(), orgID)
	if err != nil {
		logger.Error("Error with CompanyHandler.GetOrganizationProfile: ", zap.Error(err))
		if appErr, ok := err.(*apperrors.ErrorResponse); ok {
			return response.Error(c, appErr.StatusCode, appErr.Message, nil)
		}
		return response.Error(c, fiber.StatusInternalServerError, "internal server error", nil)
	...
	return response.JSON(c, fiber.StatusOK, "Organization profile retrieved successfully", profile)
}

// UpdateCompanyDetails updates editable company details
func (h *CompanyHandler) UpdateCompanyDetails(c *fiber.Ctx) error {
	orgID, _, role, ok := middlewares.GetSessionInfo(c)
	if !ok {
		return response.Error(c, fiber.StatusUnauthorized, "unauthorized", nil)
	}

	var req dto.UpdateCompanyDetailsRequest
	if err := c.BodyParser(&req); err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error(), nil)
	}
	...
	if err := h.companyServ.UpdateCompanyDetails(c.Context(), orgID, role, &req); err != nil {
		logger.Error("Error with CompanyHandler.UpdateCompanyDetails: ", zap.Error(err))
		if appErr, ok := err.(*apperrors.ErrorResponse); ok {
			return response.Error(c, appErr.StatusCode, appErr.Message, nil)
		}
		return response.Error(c, fiber.StatusInternalServerError, "internal server error", nil)
	...
	return response.JSON(c, fiber.StatusOK, "Company details updated successfully", nil)
}

// UpdateOrganizationBranding updates organization branding and logos
func (h *CompanyHandler) UpdateOrganizationBranding(c *fiber.Ctx) error {
	orgID, _, role, ok := middlewares.GetSessionInfo(c)
	if !ok {
		return response.Error(c, fiber.StatusUnauthorized, "unauthorized", nil)
	}
	...
	if err := h.companyServ.UpdateOrganizationBranding(c.Context(), orgID, role, &req); err != nil {
		logger.Error("Error with CompanyHandler.UpdateOrganizationBranding: ", zap.Error(err))
		if appErr, ok := err.(*apperrors.ErrorResponse); ok {
			return response.Error(c, appErr.StatusCode, appErr.Message, nil)
		}
		return response.Error(c, fiber.StatusInternalServerError, "internal server error", nil)
	...
	return response.JSON(c, fiber.StatusOK, "Organization branding updated successfully", nil)
}

// UploadBrandingLogo handles logo file uploads (light/dark mode) for organization branding
func (h *CompanyHandler) UploadBrandingLogo(c *fiber.Ctx) error {
	orgID, _, role, ok := middlewares.GetSessionInfo(c)
	if !ok {
		return response.Error(c, fiber.StatusUnauthorized, "unauthorized", nil)
	}

	logoType := c.FormValue("logo_type", "light")
	if logoType != "light" && logoType != "dark" {
		return response.Error(c, fiber.StatusBadRequest, "logo_type must be light or dark", nil)
	}

	fileHeader, err := c.FormFile("logo")
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, "logo file required", nil)
	}

	file, err := fileHeader.Open()
	if err != nil {
		logger.Error("Error opening logo file: ", zap.Error(err))
		return response.Error(c, fiber.StatusInternalServerError, "failed to read file", nil)
	}
	defer file.Close()

	mimeType := c.Locals("file_mime_type").(string)
	url, err := h.companyServ.UploadBrandingLogo(c.Context(), orgID, role, logoType, file, mimeType)
	if err != nil {
		logger.Error("Error with CompanyHandler.UploadBrandingLogo: ", zap.Error(err))
		if appErr, ok := err.(*apperrors.ErrorResponse); ok {
			return response.Error(c, appErr.StatusCode, appErr.Message, nil)
		}
		return response.Error(c, fiber.StatusInternalServerError, "internal server error", nil)
	...
	return response.JSON(c, fiber.StatusOK, "Logo uploaded successfully", fiber.Map{
		"logo_url":  url,
		"logo_type": logoType,
	})
}
