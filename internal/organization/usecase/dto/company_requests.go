...
type ListMembersRequest struct {
	...
	MFAEnabled *bool  `query:"mfa_enabled,omitempty"`
	Search     string `query:"search,omitempty"`
}

// UpdateMemberStatusRequest for member status updates (suspend, activate)
...
// UpdateCompanyDetailsRequest for company details section of Organization Profile
type UpdateCompanyDetailsRequest struct {
	Name                string `json:"name,omitempty" validate:"omitempty,min=2,max=100"`
	Industry            string `json:"industry,omitempty"`
	PrimaryContactEmail string `json:"primary_contact_email,omitempty" validate:"omitempty,email"`
	SupportEmail        string `json:"support_email,omitempty" validate:"omitempty,email"`
}

// UpdateOrganizationBrandingRequest for branding section of Organization Profile
