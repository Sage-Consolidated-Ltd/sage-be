...
func (s *CompanyServices) GetOrganizationByID(ctx context.Context, orgID string) (*domain.Organization, error) {
	return s.companyRepo.GetOrganizationByID(ctx, orgID)
}

func (s *CompanyServices) GetMemberByID(ctx context.Context, memberID, orgID string) (*domain.OrganizationMember, error) {
	return s.companyRepo.GetMemberByID(ctx, memberID, orgID)
}
...
func (s *CompanyServices) CanManageMembers(role string) bool {
	...
	return r == domain.OrgRoleOwner || r == domain.OrgRoleAdmin
}
...
// ResetMemberMFA resets a member's 2FA (requires owner or admin role)
func (s *CompanyServices) ResetMemberMFA(ctx context.Context, memberID, orgID, actorRole string) error {
	if !s.CanManageMembers(actorRole) {
		return apperrors.ForbiddenError("only owner or admin can reset member MFA")
	}
	return s.companyRepo.ResetMemberMFA(ctx, memberID, orgID)
}

// UpdateMemberStatus updates a member's status (active, suspended, pending)
func (s *CompanyServices) UpdateMemberStatus(ctx context.Context, memberID, orgID, actorRole, status string) error {
	if !s.CanManageMembers(actorRole) {
		return apperrors.ForbiddenError("only owner or admin can update member status")
	}
	return s.companyRepo.UpdateMemberStatus(ctx, memberID, orgID, status)
}

// GetOrganizationProfile returns the full profile (company details, system env metadata, branding)
func (s *CompanyServices) GetOrganizationProfile(ctx context.Context, orgID string) (*domain.OrganizationProfileResponse, error) {
	return s.companyRepo.GetOrganizationProfile(ctx, orgID)
}

// UpdateCompanyDetails updates editable company details (owner or admin required)
func (s *CompanyServices) UpdateCompanyDetails(ctx context.Context, orgID, actorRole string, req *dto.UpdateCompanyDetailsRequest) error {
	if !s.CanUpdateSettings(actorRole) {
		return apperrors.ForbiddenError("only owner or admin can update company details")
	}
	return s.companyRepo.UpdateCompanyDetails(ctx, orgID, req)
}

// UpdateOrganizationBranding updates branding logos and report visibility (owner or admin required)
func (s *CompanyServices) UpdateOrganizationBranding(ctx context.Context, orgID, actorRole string, req *dto.UpdateOrganizationBrandingRequest) error {
	if !s.CanUpdateSettings(actorRole) {
		return apperrors.ForbiddenError("only owner or admin can update branding settings")
	}
	return s.companyRepo.UpdateOrganizationBranding(ctx, orgID, req)
}

// UploadBrandingLogo uploads branding logo image to S3 and updates branding settings
func (s *CompanyServices) UploadBrandingLogo(ctx context.Context, orgID, actorRole, logoType string, file multipart.File, mimeType string) (string, error) {
	if !s.CanUpdateSettings(actorRole) {
		return "", apperrors.ForbiddenError("only owner or admin can upload branding logos")
	}

	if s.uploader == nil {
		return "", apperrors.InternalServerError("storage service uninitialized")
	}

	url, _, err := s.uploader.UploadLogo(ctx, file, orgID, logoType, mimeType)
	if err != nil {
		return "", err
	}

	req := &dto.UpdateOrganizationBrandingRequest{}
	if logoType == "light" {
		req.LogoLightURL = url
	} else {
		req.LogoDarkURL = url
	}

	if err := s.companyRepo.UpdateOrganizationBranding(ctx, orgID, req); err != nil {
		return "", err
	}
